...
# plt.gcf().set_size_inches(30/2.54, 8/2.54)
...
for i in range(3):
    ax = axes[i]
    extent = []
    im = ax.imshow(np.rot90(contrast_set[i]),extent=[-10,10,-200,200], origin='lower', cmap='jet')
    ax.set_aspect(1/20)
    ax.set_ylabel('角度偏差$\mathrm{(\u03BCrad)}$')
    ax.set_xlabel('平移量$\mathrm{(mm)}$')
colorbar = fig2.colorbar(im, ax=ax6, fraction=0.046, pad=0.04)
colorbar.set_label('对比度')
plt.tight_layout()
# plt.show()   
