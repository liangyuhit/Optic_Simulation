...
for m in range(16):
    ...
    V_x, V_y, Vz = 0.0002*633/600*1.8, 0+1e-10, 1
    ...
    fit_xaxis = fit_yaxis = np.linspace(0, 4, 1001)
...
plt.subplot(2,4,1)
plt.text(0, 0, 'V_x_2 from 0 to 0.000015', fontsize=10, style='oblique')
plt.xticks([])
plt.yticks([])
plt.axis('off')
...
for i in range(len(dx)*len(dy)-1):
    ...
    plt.xlabel('V_x_2')
    plt.legend(loc='best')
plt.savefig('C:/Users/yu03/Desktop/Gabor/Interferometer Sim/Simulation_Result/1. V_x_2 Modulation/Compare_VmeaX.jpg', dpi=300)
plt.close()
